...
void PiccReset(void)
{
    PcdAntenna_RFID_6(OFF);
    Delay_ms(20);

    PcdAntenna_RFID_6(ON);
    Delay_ms(20);
}
...
unsigned char PiccRequest(unsigned char cReq, unsigned char *cATQA)
{
    ...
    PcdConfig_RFID_6(PCD_TYPE_14443A);

    PcdSetTmo_RFID_6(0);

    SetBitMask_RFID_6(PHHAL_HW_RFID_6_REG_DRVMODE, 0x08); // Enable Transmitter PIN
    Write_RFID_6(PHHAL_HW_RFID_6_REG_TXCRCCON, 0x18);     // Disable TX CRC
    Write_RFID_6(PHHAL_HW_RFID_6_REG_RXCRCCON, 0x18);     // Disable RX CRC
    Write_RFID_6(PHHAL_HW_RFID_6_REG_TXDATANUM, 0x0F);    // Sent last data is 7 bit; Need to reset.
    Write_RFID_6(PHHAL_HW_RFID_6_REG_RXBITCTRL, 0x00);

    SetBitMask_RFID_6(PHHAL_HW_RFID_6_REG_FRAMECON, 0xC0);
    ClearBitMask_RFID_6(PHHAL_HW_RFID_6_REG_STATUS, 0x20);

    g_sPcdTrsv.cData[0] = cReq;
    g_sPcdTrsv.iLength  = 1;

    cStatus = PcdTransceive_RFID_6(PCD_CMD_TRANSCEIVE);
...
unsigned char PiccSelect(unsigned char cLev, unsigned char *cUID, unsigned char *cSAK)
{
    ...
    Write_RFID_6(PHHAL_HW_RFID_6_REG_TXCRCCON, 0x19);
    Write_RFID_6(PHHAL_HW_RFID_6_REG_RXCRCCON, 0x19);
    Write_RFID_6(PHHAL_HW_RFID_6_REG_RXBITCTRL, 0x00);
...
unsigned char PiccHaltA(void)
{
    ...
    g_sPcdTrsv.iLength  = 0x02;
    cStatus             = PcdTransceive_RFID_6(PCD_CMD_TRANSMIT);
...
{
    if (cWTX == 0 || cWTX > 59)
        PcdSetTmo_RFID_6(cTimeOut);
    // else
    // Write_RFID_6(JREG_TRELOADLO, 0x04 * cWTX + 1);
...
unsigned char ISO14443_COS(unsigned short *cpLen, unsigned char *cpAPDU)
{
    ...
    PcdSetTmo_RFID_6(g_sPcdPara.cFWI);
    ...
    do {
        ...
        //{
        cStatus = PcdTransceive_RFID_6(PCD_CMD_TRANSCEIVE); // 收发正常，继续循环
...
unsigned char PiccRequestB(unsigned char cReq, unsigned char AFI, unsigned char N, unsigned char *cpATQB)
{
    ...
    PcdConfig_RFID_6(PCD_TYPE_14443B);
    ...
    if (cStatus == ERR_SUCCESS && g_sPcdTrsv.iLength == 0x0C) {
        ...
        // PcdSetTmo_RFID_6(cpATQB[11]>>4); // set FWT
    } else
        cStatus = ERR_FAILURE;
...
uint8_t PiccAuthState(uint8_t cSector, uint8_t cMode, uint8_t* key)
{
    ...
    cStatus = PcdTransceive_RFID_6(PCD_CMD_LOADKEY);
    ...
    if (cStatus == ERR_SUCCESS) {
        ...
        cStatus = PcdTransceive_RFID_6(PCD_CMD_MFAUTHENT);
    }

    if (cStatus == ERR_SUCCESS) {
        // cStatus = Read_RFID_6(JREG_STATUS2); // WAIT ADD

        if (Read_RFID_6(PHHAL_HW_RFID_6_REG_STATUS) & 0x20)
            cStatus = ERR_SUCCESS;
        else
            cStatus = MI_AUTHERR;
    }
