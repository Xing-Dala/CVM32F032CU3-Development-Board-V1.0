 ...
 * @brief RFID_6 Register and command
 * @version 1.0
 * @date 2022-10-11
 *
...
#define RFID_6_RST_Pin   4 // PDOWN

#define RFID_6_CLK_Pin   5  // SCL
#define RFID_6_SDA_Pin   6  // SDA
#define RFID_6_ADR1_Pin  21 // ADR1
#define RFID_6_ADR2_Pin  1  // ADR2

#define RFID_6_IRQ_Pin   7 // IRQ

#define RFID_6_SL0_Pin   20 // SL0
#define RFID_6_SL1_Pin   0  // SL1

#define RFID_6_SDA_Hight gpio_output_high(RFID_6_SDA_Pin)
#define RFID_6_CLK_Hight gpio_output_high(RFID_6_CLK_Pin)
#define RFID_6_RST_Hight gpio_output_high(RFID_6_RST_Pin)

#define RFID_6_SDA_Low   gpio_output_low(RFID_6_SDA_Pin)
#define RFID_6_CLK_Low   gpio_output_low(RFID_6_CLK_Pin)
#define RFID_6_RST_Low   gpio_output_low(RFID_6_RST_Pin)

#define RFID_6_SDA_Read  IO_Read(RFID_6_SDA_Pin)
#define RFID_6_IRQ_Read  IO_Read(RFID_6_IRQ_Pin)

/**
 * @brief RFID_6 Command
 *
 */
...
#define PCD_LOADREG      0x0C // reads data from the internal EEPROM and initializes the CLRFID_6 registers. EEPROM address needs to be within EEPROM sector 2
#define PCD_LOADPROTOCOL 0x0D // reads data from the internal EEPROM and initializes the CLRFID_6 registers needed for a Protocol change
...
#define PCD_SOFTRESET    0x1F // resets the CLRFID_6

/**
 * @brief RFID_6 Register
 *
 */
...
// RFID_6 Command Register Contents (0x00)
...
#define PHHAL_HW_RFID_6_REG_COMMAND         0x00 // Command Register
#define PHHAL_HW_RFID_6_REG_HOSTCTRL        0x01 // Configure Host and SAM interfaces.
#define PHHAL_HW_RFID_6_REG_FIFOCONTROL     0x02 // FIFO-Control Register
#define PHHAL_HW_RFID_6_REG_WATERLEVEL      0x03 // FIFO WaterLevel configuration
#define PHHAL_HW_RFID_6_REG_FIFOLENGTH      0x04 // FIFO-Length Register
#define PHHAL_HW_RFID_6_REG_FIFODATA        0x05 // FIFO-Data Register
#define PHHAL_HW_RFID_6_REG_IRQ0            0x06 // IRQ Registers
#define PHHAL_HW_RFID_6_REG_IRQ1            0x07 // IRQ1 Register
#define PHHAL_HW_RFID_6_REG_IRQ0EN          0x08 // IRQ0EN Register
#define PHHAL_HW_RFID_6_REG_IRQ1EN          0x09 // IRQ1EN Register.
#define PHHAL_HW_RFID_6_REG_ERROR           0x0A // Error Register.
#define PHHAL_HW_RFID_6_REG_STATUS          0x0B // Status Register
#define PHHAL_HW_RFID_6_REG_RXBITCTRL       0x0C // Rx-Bit-Control Register
#define PHHAL_HW_RFID_6_REG_RXCOLL          0x0D // Rx-Coll Register
#define PHHAL_HW_RFID_6_REG_TCONTROL        0x0E // Timer Registers
#define PHHAL_HW_RFID_6_REG_T0CONTROL       0x0F // Timer0 Control Register
#define PHHAL_HW_RFID_6_REG_T0RELOADHI      0x10 // Timer0 Reload(High) Register
#define PHHAL_HW_RFID_6_REG_T0RELOADLO      0x11 // Timer0 Reload(Low) Register
#define PHHAL_HW_RFID_6_REG_T0COUNTERVALHI  0x12 // Timer0 Counter(High) Register
#define PHHAL_HW_RFID_6_REG_T0COUNTERVALLO  0x13 // Timer0 Counter(Low) Register
#define PHHAL_HW_RFID_6_REG_T1CONTROL       0x14 // Timer1 Control Register
#define PHHAL_HW_RFID_6_REG_T1RELOADHI      0x15 // Timer1 Reload(High) Register
#define PHHAL_HW_RFID_6_REG_T1RELOADLO      0x16 // Timer1 Reload(Low) Register
#define PHHAL_HW_RFID_6_REG_T1COUNTERVALHI  0x17 // Timer1 Counter(High) Register
#define PHHAL_HW_RFID_6_REG_T1COUNTERVALLO  0x18 // Timer1 Counter(Low) Register
#define PHHAL_HW_RFID_6_REG_T2CONTROL       0x19 // Timer2 Control Register
#define PHHAL_HW_RFID_6_REG_T2RELOADHI      0x1A // Timer2 Reload(High) Register
#define PHHAL_HW_RFID_6_REG_T2RELOADLO      0x1B // Timer2 Reload(Low) Register
#define PHHAL_HW_RFID_6_REG_T2COUNTERVALHI  0x1C // Timer2 Counter(High) Register
#define PHHAL_HW_RFID_6_REG_T2COUNTERVALLO  0x1D // Timer2 Counter(Low) Register
#define PHHAL_HW_RFID_6_REG_T3CONTROL       0x1E // Timer3 Control Register
#define PHHAL_HW_RFID_6_REG_T3RELOADHI      0x1F // Timer3 Reload(High) Register
#define PHHAL_HW_RFID_6_REG_T3RELOADLO      0x20 // Timer3 Reload(Low) Register
#define PHHAL_HW_RFID_6_REG_T3COUNTERVALHI  0x21 // Timer3 Counter(High) Register
#define PHHAL_HW_RFID_6_REG_T3COUNTERVALLO  0x22 // Timer3 Counter(Low) Register
#define PHHAL_HW_RFID_6_REG_T4CONTROL       0x23 // Timer4 Control Register
#define PHHAL_HW_RFID_6_REG_T4RELOADHI      0x24 // Timer4 Reload(High) Register
#define PHHAL_HW_RFID_6_REG_T4RELOADLO      0x25 // Timer4 Reload(Low) Register
#define PHHAL_HW_RFID_6_REG_T4COUNTERVALHI  0x26 // Timer4 Counter(High) Register.
#define PHHAL_HW_RFID_6_REG_T4COUNTERVALLO  0x27 // Timer4 Counter(Low) Register
#define PHHAL_HW_RFID_6_REG_DRVMODE         0x28 // Driver Mode Register
#define PHHAL_HW_RFID_6_REG_TXAMP           0x29 // Tx Amplifier Register
#define PHHAL_HW_RFID_6_REG_DRVCON          0x2A // Driver Control Register.
#define PHHAL_HW_RFID_6_REG_TXI             0x2B // TxI Register
#define PHHAL_HW_RFID_6_REG_TXCRCCON        0x2C // Tx-CRC Control Register
#define PHHAL_HW_RFID_6_REG_RXCRCCON        0x2D // Rx-CRC Control Register
#define PHHAL_HW_RFID_6_REG_TXDATANUM       0x2E // Tx-DataNum Register
#define PHHAL_HW_RFID_6_REG_TXMODWIDTH      0x2F // Tx-Modwidth Register
#define PHHAL_HW_RFID_6_REG_TXSYM10BURSTLEN 0x30 // Symbol 0 and 1 Register
#define PHHAL_HW_RFID_6_REG_TXWAITCTRL      0x31 // Tx-Wait Control Register
#define PHHAL_HW_RFID_6_REG_TXWAITLO        0x32 // TxWaitLo Register
#define PHHAL_HW_RFID_6_REG_FRAMECON        0x33 // Frame control register
#define PHHAL_HW_RFID_6_REG_RXSOFD          0x34 // RxSOFD Register
#define PHHAL_HW_RFID_6_REG_RXCTRL          0x35 // Rx Control Register
#define PHHAL_HW_RFID_6_REG_RXWAIT          0x36 // Rx-Wait Register
#define PHHAL_HW_RFID_6_REG_RXTHRESHOLD     0x37 // Rx-Threshold Register
#define PHHAL_HW_RFID_6_REG_RCV             0x38 // Receiver Register
#define PHHAL_HW_RFID_6_REG_RXANA           0x39 // Rx-Analog Register
#define PHHAL_HW_RFID_6_REG_SERIALSPEED     0x3B // Serial Speed Register.
#define PHHAL_HW_RFID_6_REG_LPO_TRIMM       0x3C // LPO_TRIMM Register
#define PHHAL_HW_RFID_6_REG_PLL_CTRL        0x3D // PLL Control Register.
#define PHHAL_HW_RFID_6_REG_PLL_DIV         0x3E // PLL DivO Register
#define PHHAL_HW_RFID_6_REG_LPCD_QMIN       0x3F // LPCD QMin Register.
#define PHHAL_HW_RFID_6_REG_LPCD_QMAX       0x40 // LPCD QMax Register
#define PHHAL_HW_RFID_6_REG_LPCD_IMIN       0x41 // LPCD IMin Register
#define PHHAL_HW_RFID_6_REG_LPCD_RESULT_I   0x42 // LPCD Result(I) Register
#define PHHAL_HW_RFID_6_REG_LPCD_RESULT_Q   0x43 // LPCD Result(Q) Register
#define PHHAL_HW_RFID_6_REG_VERSION         0x7F // Version Register

// RFID_6 Command Register Contents (0x00)
#define PHHAL_HW_RFID_6_BIT_STANDBY      0x80U // 备用位；如果设置，IC将转换为待机模式

#define PHHAL_HW_RFID_6_CMD_IDLE         0x00U // 无动作；取消当前命令执行。
#define PHHAL_HW_RFID_6_CMD_LPCD         0x01U // 低功耗卡检测
#define PHHAL_HW_RFID_6_CMD_LOADKEY      0x02U // 自FIFO缓冲器读取一个 MIFARE 密钥（6个字节大小），并放入密钥缓冲器中
#define PHHAL_HW_RFID_6_CMD_MFAUTHENT    0x03U // 仅以MIFARE读/写模式执行 MIFARE 标准认证
#define PHHAL_HW_RFID_6_CMD_ACKREQ       0x04U // 执行查询，用于 ISO/IEC 18000-3 Mode 3/ EPC Class-1 HF 的 Ack 及 Req-Rn
#define PHHAL_HW_RFID_6_CMD_RECEIVE      0x05U // 启动接收电路
#define PHHAL_HW_RFID_6_CMD_TRANSMIT     0x06U // 由 FIFO 缓冲器传输数据
#define PHHAL_HW_RFID_6_CMD_TRANSCEIVE   0x07U // 由 FIFO 缓冲器传输数据，且在传输结束后自动启动接收器
#define PHHAL_HW_RFID_6_CMD_WRITEE2      0x08U // 自 FIFO 缓冲器取得一个字节，并将其写入内部 EEPROM 中。有效的地址范围限于 MIFARE 密钥区内的地址
#define PHHAL_HW_RFID_6_CMD_WRITEE2PAGE  0x09U // 自 FIFO 缓冲器中取得最多达 64 个字节（一页 EEPROM 页面）并将其写入 EEPROM 中。有效的地址范围限于MIFARE 密钥区内的地址
#define PHHAL_HW_RFID_6_CMD_READE2       0x0AU // 自 EEPROM 读取数据并将其复制到FIFO 缓冲器，有效的地址范围限于 MIFARE 密钥区内的地址
#define PHHAL_HW_RFID_6_CMD_LOADREG      0x0CU // 从 EEPROM 内部读取数据，并初始化CLRC663 寄存器。EEPROM 地址必须在EEPROM 第 2 区内
#define PHHAL_HW_RFID_6_CMD_LOADPROTOCOL 0x0DU // 从 EEPROM 内部读取数据，并初始化改变协议所需的 CLRC663 寄存器
#define PHHAL_HW_RFID_6_CMD_LOADKEYE2    0x0EU // 复制EEPROM的一个密钥到密钥缓冲区
#define PHHAL_HW_RFID_6_CMD_STOREKEYE2   0x0FU // 存储MIFARE密钥（6字节大小）到EEPROM
#define PHHAL_HW_RFID_6_CMD_SOFTRESET    0x1FU // 复位 CLRC663

#define PHHAL_HW_RFID_6_MASK_COMMAND     0x1FU // 命令位掩码

/**
 * @brief RFID_6 command redefinition
 *
 */
#define PCD_CMD_LOADKEY    PHHAL_HW_RFID_6_CMD_LOADKEY
#define PCD_CMD_MFAUTHENT  PHHAL_HW_RFID_6_CMD_MFAUTHENT
#define PCD_CMD_RECEIVE    PHHAL_HW_RFID_6_CMD_RECEIVE
#define PCD_CMD_TRANSMIT   PHHAL_HW_RFID_6_CMD_TRANSMIT
#define PCD_CMD_TRANSCEIVE PHHAL_HW_RFID_6_CMD_TRANSCEIVE

/**
 * @brief RFID_6 Error Code Definition
 *
 */
#define MI_OK                     0x00
...
#define PCD_CMD_LOADKEY           PHHAL_HW_RFID_6_CMD_LOADKEY    // 自FIFO缓冲器读取一个 MIFARE 密钥（6个字节大小），并放入密钥缓冲器中
#define PCD_CMD_MFAUTHENT         PHHAL_HW_RFID_6_CMD_MFAUTHENT  // 仅以MIFARE读/写模式执行 MIFARE 标准认证
#define PCD_CMD_RECEIVE           PHHAL_HW_RFID_6_CMD_RECEIVE    // 启动接收电路
#define PCD_CMD_TRANSMIT          PHHAL_HW_RFID_6_CMD_TRANSMIT   // 由 FIFO 缓冲器传输数据
#define PCD_CMD_TRANSCEIVE        PHHAL_HW_RFID_6_CMD_TRANSCEIVE // 由 FIFO 缓冲器传输数据，且在传输结束后自动启动接收器
...
void RFID_6_IIC_Init(void);
void RFID_6_Init(void);
unsigned char Read_RFID_6(unsigned char Address);
void Write_RFID_6(unsigned char Address, unsigned char cValue);
void SetBitMask_RFID_6(unsigned char cAddr, unsigned char cMask);
void ClearBitMask_RFID_6(unsigned char cAddr, unsigned char cMask);

unsigned char PcdReset_RFID_6(void);
unsigned char PcdConfig_RFID_6(unsigned char cType);
unsigned char PcdTransceive_RFID_6(unsigned char cCommand);

void PcdSetTmo_RFID_6(unsigned char tmoLength);
void PcdSetFSD_RFID_6(unsigned char cFSDI);

void PcdAntenna_RFID_6(unsigned char cSta);

void RFID_6_Read_Card(void);
