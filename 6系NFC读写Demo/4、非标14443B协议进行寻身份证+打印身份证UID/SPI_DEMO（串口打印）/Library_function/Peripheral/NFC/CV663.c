 ...
 *
 */
#define Analog_SIP // 模拟SPI
// #define High_Speed_SPI // 高速SPI
...
{
    uint8_t ATQB[] = 0;
    uint8_t cStatus = PiccRequestB(0x00, 0x00, 0x00, ATQB);

    if (cStatus == ERR_SUCCESS)
    {
        printf("\r\nWrite REQB:0x05 0x00 0x00");
        PcdSetTmo_CV663(g_sPcdPara.cFWI);
        g_sPcdTrsv.cData[0] = 0x05;
        g_sPcdTrsv.cData[1] = 0x00;
        g_sPcdTrsv.cData[2] = 0x00;
        g_sPcdTrsv.iLength = 0x03;
        PcdTransceive_CV663(PCD_CMD_TRANSCEIVE);
        printf("\r\nRead ATQB:");
        for (uint8_t i = 0; i < 12; i++)
            printf("0x%02X ", g_sPcdTrsv.cData[i]);
        printf("\r\n");

        printf("\r\nWrite Attrib:0x1d 0x00 0x00 0x00 0x00 0x00 0x08 0x01 0x08");
        PcdSetTmo_CV663(g_sPcdPara.cFWI);
        g_sPcdTrsv.cData[0] = 0x1d;
        g_sPcdTrsv.cData[1] = 0x00;
        g_sPcdTrsv.cData[2] = 0x00;
        g_sPcdTrsv.cData[3] = 0x00;
        g_sPcdTrsv.cData[4] = 0x00;
        g_sPcdTrsv.cData[5] = 0x00;
        g_sPcdTrsv.cData[6] = 0x08;
        g_sPcdTrsv.cData[7] = 0x01;
        g_sPcdTrsv.cData[8] = 0x08;
        g_sPcdTrsv.iLength = 0x09;
        PcdTransceive_CV663(PCD_CMD_TRANSCEIVE);
        printf("\r\nRead answer:0x%02X\r\n", g_sPcdTrsv.cData[0]);

        printf("\r\nWrite R_UID:0x00 0x36 0x00 0x00 0x08");
        PcdSetTmo_CV663(g_sPcdPara.cFWI);
        g_sPcdTrsv.cData[0] = 0x00;
        g_sPcdTrsv.cData[1] = 0x36;
        g_sPcdTrsv.cData[2] = 0x00;
        g_sPcdTrsv.cData[3] = 0x00;
        g_sPcdTrsv.cData[4] = 0x08;
