...
#define PCD_LOADREG      0x0C // reads data from the internal EEPROM and initializes the CV663 registers. EEPROM address needs to be within EEPROM sector 2
#define PCD_LOADPROTOCOL 0x0D // reads data from the internal EEPROM and initializes the CV663 registers needed for a Protocol change
...
#define PCD_SOFTRESET    0x1F // resets the CV663
...
#define PHHAL_HW_CV663_REG_WATERLEVEL      0x03 // 定义用于 FIFO 的下溢及上溢警告级别。如果 512 位 FIFO 模式通过设置 FIFOControl.FIFOSize 位被启动，则需要运用 FIFOControl 中的一个设置位来扩展水位定义。
